...
def test_TestMustDecodeBase58Address():
    ...
    assert skycoin.SKY_cipher_MustDecodeBase58Address(
        h, addres) == skycoin.SKY_OK
...
def test_TestDecodeBase58Address():
    ...
    assert skycoin.SKY_cipher_DecodeBase58Address(
        h, addres_2) == skycoin.SKY_OK
    assert addres == addres_2

    _, addres_str = skycoin.SKY_cipher_Address_String(addres)
    assert skycoin.SKY_cipher_DecodeBase58Address(
        addres_str, addres_2) == skycoin.SKY_OK
    assert addres == addres_2
...
def test_TestAddressFromBytes():
    ...
    addres_2 = skycoin.cipher__Address()
    _, byte = skycoin.SKY_cipher_Address_Bytes(addres)
    assert skycoin.SKY_cipher_AddressFromBytes(
        byte, addres_2) == skycoin.SKY_OK
    assert addres == addres_2

    # Invalid number of bytes
    __ = skycoin.cipher__Address()
    assert skycoin.SKY_cipher_AddressFromBytes(
        byte[:len(byte) - 2], __) != 0  # "Invalid address length"
...
def test_TestBitcoinAddressFromBytes():
    ...
    assert skycoin.SKY_cipher_BitcoinAddressFromBytes(
        byte, addres_2) == skycoin.SKY_OK
...
def test_TestAddressString():
    ...
    assert skycoin.SKY_cipher_DecodeBase58Address(
        addres_2_str, addres_3) == skycoin.SKY_OK
...
def test_TestBitcoinWIPRoundTrio():
    ...
    _, wip_1 = skycoin.SKY_cipher_BitcoinWalletImportFormatFromSeckey(
        secret_key_1)
    assert skycoin.SKY_cipher_SecKeyFromWalletImportFormat(
        wip_1, secret_key_2) == skycoin.SKY_OK
    _, wip_2 = skycoin.SKY_cipher_BitcoinWalletImportFormatFromSeckey(
        secret_key_2)
...
def test_TestBitcoinWIP():
    ...
    for p in range(len(wips)):
        ...
        public_key = skycoin.cipher_PubKey()
        assert skycoin.SKY_cipher_SecKeyFromWalletImportFormat(
            wips[p], secret_key) == skycoin.SKY_OK
        skycoin.SKY_cipher_PubKeyFromSecKey(secret_key, public_key)
        _, public_key_hex = skycoin.SKY_cipher_PubKey_Hex(public_key)
        assert public_key_hex == publics[p]
        _, bitcoin_addr = skycoin.SKY_cipher_BitcoinAddressFromPubkey(
            public_key)
...
def test_TestAddressBulk():
    for _ in range(1024):
        ...
        assert skycoin.SKY_cipher_Address_Verify(
            addres_1, public_key) == skycoin.SKY_OK
