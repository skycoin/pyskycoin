...
def test_TestAddUint64():
    err, n = skycoin.coin.AddUint64(10, 11)
    assert err == skycoin.SKY_OK
    assert int(21) == n
    err, n = skycoin.coin.AddUint64(int(0xFFFFFFFFFFFFFFFF), 1)
...
def test_TestUint64ToInt64():
    ...
    for val in cases:
        s = int(val.a)
        err, x = skycoin.coin.Uint64ToInt64(s)
...
def test_TestInt64ToUint64():
    ...
    for val in cases:
        s = int(val.a)
        err, x = skycoin.coin.Int64ToUint64(s)
        if err != skycoin.SKY_OK:
            assert skycoin.SKY_ErrUint64AddOverflow == val.err
        else:
            assert val.b == x
