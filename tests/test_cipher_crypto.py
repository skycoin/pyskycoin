...
def test_TestNewPubKey():
    ...
    assert skycoin.SKY_cipher_NewPubKey(data, public_key) == error["SKY_ERROR"]
    ...
    assert skycoin.SKY_cipher_NewPubKey(data, public_key) == error["SKY_OK"]
...
def test_TestPubKeyFromHex():
    ...
    # Invalid hex
    assert skycoin.SKY_cipher_PubKeyFromHex(b'""', public_key) == error["SKY_ERROR"]
    assert skycoin.SKY_cipher_PubKeyFromHex(b'"cascs"', public_key) == error["SKY_ERROR"]
    # Invalid hex length
    _, data = skycoin.SKY_cipher_RandByte(33)
    skycoin.SKY_cipher_NewPubKey(data, public_key)
    _, public_key_hex = skycoin.SKY_cipher_PubKey_Hex(public_key)
    assert skycoin.SKY_cipher_MustPubKeyFromHex(public_key_hex[:int(len(public_key_hex) / 2)], public_key) == error["SKY_ERROR"]
    # Valid
    assert skycoin.SKY_cipher_MustPubKeyFromHex(public_key_hex, public_key_2) == error["SKY_OK"]
...
def test_TestPubKeyVerifyNil():
    ...
    public_key = skycoin.cipher_PubKey()
    assert skycoin.SKY_cipher_PubKey_Verify(public_key) == error["SKY_ERROR"]
...
def test_TestPubKeyVerifyDefault1():
    ...
    assert skycoin.SKY_cipher_PubKey_Verify(public_key) == error["SKY_OK"]
...
def test_TestPubKeyToAddress():
    ...
    assert skycoin.SKY_cipher_DecodeBase58Address(addres_str, addres) == error["SKY_OK"]
...
def test_TestMustNewSecKey():
    secret_key = skycoin.cipher_SecKey()
    _, data = skycoin.SKY_cipher_RandByte(31)
    assert skycoin.SKY_cipher_NewSecKey(data, secret_key) == error["SKY_ERROR"]
    _, data = skycoin.SKY_cipher_RandByte(33)
    assert skycoin.SKY_cipher_NewSecKey(data, secret_key) == error["SKY_ERROR"]
    _, data = skycoin.SKY_cipher_RandByte(34)
    assert skycoin.SKY_cipher_NewSecKey(data, secret_key) == error["SKY_ERROR"]
    ...
    assert skycoin.SKY_cipher_NewSecKey(data, secret_key) == error["SKY_ERROR"]
    _, data = skycoin.SKY_cipher_RandByte(32)
    assert skycoin.SKY_cipher_NewSecKey(data, secret_key) == error["SKY_OK"]
...
def test_TestMustSecKeyFromHex():
    ...
    # Invalid hex
    assert skycoin.SKY_cipher_SecKeyFromHex(b'""', secret_key) == error["SKY_ERROR"]
    assert skycoin.SKY_cipher_SecKeyFromHex(b'cascs', secret_key) == error["SKY_ERROR"]
    # Invalid hex length
    _, data = skycoin.SKY_cipher_RandByte(32)
    skycoin.SKY_cipher_NewSecKey(data, secret_key)
    _, secret_key_hex = skycoin.SKY_cipher_SecKey_Hex(secret_key)
    assert skycoin.SKY_cipher_MustSecKeyFromHex(secret_key_hex[:int(len(secret_key_hex) / 2)], secret_key) == error["SKY_ERROR"] 
    # Valid
    assert skycoin.SKY_cipher_MustSecKeyFromHex(secret_key_hex, secret_key_2) == error["SKY_OK"]
...
def test_TestNewSig():
    ...
    assert skycoin.SKY_cipher_NewSig(data, sig) == error["SKY_ERROR"]
    ...
    assert skycoin.SKY_cipher_NewSig(data, sig) == error["SKY_OK"]
...
def test_TestMustSigFromHex():
    ...
    # Invalid hex
    assert skycoin.SKY_cipher_SigFromHex(b"", sig_1) == error["SKY_ERROR"]
    assert skycoin.SKY_cipher_SigFromHex(b"cascs", sig_1) == error["SKY_ERROR"]
    # Invalid hex length
    _, data = skycoin.SKY_cipher_RandByte(65)
    skycoin.SKY_cipher_NewSig(data, sig_1)
    _, sig_1_hex = skycoin.SKY_cipher_Sig_Hex(sig_1)
    assert skycoin.SKY_cipher_MustSigFromHex(sig_1_hex[:int(len(sig_1_hex) / 2)], sig_1) == error["SKY_ERROR"]
    # Valid 
    assert skycoin.SKY_cipher_MustSigFromHex(sig_1_hex, sig_2) == error["SKY_OK"]
    assert sig_1 == sig_2 
...
def test_TestChkSig():
    ...
    assert skycoin.SKY_cipher_ChkSig(addres, sha_sum, sig_1) == error["SKY_OK"]
    # Empty sig should be invalid
    assert skycoin.SKY_cipher_ChkSig(addres, sha_sum, sig_2) == error["SKY_ERROR"]
    # Random sigs should not pass
    for _ in range(100):
        _, data = skycoin.SKY_cipher_RandByte(65)
        skycoin.SKY_cipher_NewSig(data, sig_1)
        assert skycoin.SKY_cipher_ChkSig(addres, sha_sum, sig_1) == error["SKY_ERROR"]
    ...
    assert skycoin.SKY_cipher_ChkSig(addres, sha_sum_2, sig_2) == error["SKY_OK"]
    assert skycoin.SKY_cipher_ChkSig(addres, sha_sum, sig_2) == error["SKY_ERROR"]
    assert skycoin.SKY_cipher_ChkSig(addres, sha_sum_2, sig_1) == error["SKY_ERROR"]
    ...
    assert skycoin.SKY_cipher_ChkSig(addres, sha_sum, sig_1) == error["SKY_OK"]
    assert skycoin.SKY_cipher_ChkSig(addres_2, sha_sum, sig_2) == error["SKY_OK"]
    assert sig_1 != sig_2
    _, data = skycoin.SKY_cipher_RandByte(256)
    skycoin.SKY_cipher_SumSHA256(data, sha_sum)
    skycoin.SKY_cipher_SignHash(sha_sum, secret_key_1, sig_1)
    ...
    assert skycoin.SKY_cipher_ChkSig(addres, sha_sum, sig_2) == error["SKY_ERROR"]
    assert skycoin.SKY_cipher_ChkSig(addres_2, sha_sum, sig_1) == error["SKY_ERROR"]
...
def test_TestPubKeyFromSecKey():
    ...
    assert public_key == public_key_2
    secret_key_2 = skycoin.cipher_SecKey()
    assert skycoin.SKY_cipher_PubKeyFromSecKey(secret_key_2, public_key) == error["SKY_ERROR"]
...
def test_TestPubKeyFromSig():
    ...
    skycoin.SKY_cipher_SignHash(sha_sum, secret_key, sig_1)
    public_key_2 = skycoin.cipher_PubKey()
    assert skycoin.SKY_cipher_PubKeyFromSig(sig_1, sha_sum, public_key_2) == error["SKY_OK"]
    assert public_key == public_key_2
    sig_2 = skycoin.cipher_Sig()
    assert skycoin.SKY_cipher_PubKeyFromSig(sig_2, sha_sum, public_key_2) == error["SKY_ERROR"]
...
def test_TestVerifySignature():
    ...
    assert skycoin.SKY_cipher_VerifySignature(public_key, sig_1, sha_sum_1) == error["SKY_OK"]
    assert skycoin.SKY_cipher_VerifySignature(public_key, sig_2, sha_sum_1) == error["SKY_ERROR"]
    assert skycoin.SKY_cipher_VerifySignature(public_key, sig_1, sha_sum_2) == error["SKY_ERROR"]
    public_key_2 = skycoin.cipher_PubKey()
    skycoin.SKY_cipher_GenerateKeyPair(public_key_2, secret_key)
    assert skycoin.SKY_cipher_VerifySignature(public_key_2, sig_1, sha_sum_1) == error["SKY_ERROR"]
    public_key_3 = skycoin.cipher_PubKey()
    assert skycoin.SKY_cipher_VerifySignature(public_key_3, sig_1, sha_sum_1) == error["SKY_ERROR"]
...
def test_TestGenerateKeyPair():
    ...
    skycoin.SKY_cipher_GenerateKeyPair(public_key, secret_key)
    assert skycoin.SKY_cipher_PubKey_Verify(public_key) == error["SKY_OK"]
    assert skycoin.SKY_cipher_SecKey_Verify(secret_key) == error["SKY_OK"]
...
def test_TestSecKeTest():
    ...
    secret_key_2 = skycoin.cipher_SecKey()
    skycoin.SKY_cipher_GenerateKeyPair(public_key, secret_key)
    assert skycoin.SKY_cipher_TestSecKey(secret_key) == error["SKY_OK"]
    assert skycoin.SKY_cipher_TestSecKey(secret_key_2) == error["SKY_ERROR"]
...
def test_TestSecKeyHashTest():
    ...
    _, data = skycoin.SKY_cipher_RandByte(256)
    skycoin.SKY_cipher_SumSHA256(data, sha_sum_1)
    assert skycoin.SKY_cipher_TestSecKeyHash(secret_key, sha_sum_1) == error["SKY_OK"]
    assert skycoin.SKY_cipher_TestSecKeyHash(secret_key_2, sha_sum_1) == error["SKY_ERROR"]
