...
def test_TestFeInv():
    ...
    assert skycoin.SKY_secp256k1go_Field_SetHex(in_, in_hex) == error["SKY_OK"]
    assert skycoin.SKY_secp256k1go_Field_SetHex(exp, exp_hex) == error["SKY_OK"]
    assert skycoin.SKY_secp256k1go_Field_Inv(in_, out_) == error["SKY_OK"]
    assert out_ == exp
