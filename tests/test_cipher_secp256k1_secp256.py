...
def test_Test_Secp256_00():
    err , nonce = skycoin.cipher.RandByte(32)
...
def test_Test_Secp256_01():
    ...
    assert skycoin.cipher.GenerateKeyPair(pubkey, seckey) == skycoin.SKY_OK
    assert skycoin.cipher.PubKeyVerify(pubkey) == skycoin.SKY_OK
    assert skycoin.cipher.SecKeyVerify(seckey) == skycoin.SKY_OK


def test_Test_PubkeyFromSeckey():
    _, privkey = skycoin.base58.String2Hex(b"f19c523315891e6e15ae0608a35eec2e00ebd6d1984cf167f46336dabd9b2de4")
    _, desiredPubKey = skycoin.base58.String2Hex(b"04fe43d0c2c3daab30f9472beb5b767be020b81c7cc940ed7a7e910f0c1d9feef10fe85eb3ce193405c2dd8453b7aeb6c1752361efdbf4f52ea8bf8f304aab37ab")

    err, pubkey = skycoin.secp256k1.UncompressedPubkeyFromSeckey(privkey)
    assert err == skycoin.SKY_OK
    assert pubkey != None
...
def RandX():
    err, pubkey, seckey = skycoin.secp256k1.GenerateKeyPair()
    assert err == skycoin.SKY_OK
    err, msg = skycoin.cipher.RandByte(32)
    assert err == skycoin.SKY_OK
    err, sig = skycoin.secp256k1.Sign(msg, seckey)
    assert err == skycoin.SKY_OK
    return pubkey, seckey, msg, sig


def test_Test_SignatureVerifyPubkey():
    err, pubkey1, seckey = skycoin.secp256k1.GenerateKeyPair()
    assert err == skycoin.SKY_OK
    err, msg = skycoin.cipher.RandByte(32)
    assert err == skycoin.SKY_OK
    err, sig = skycoin.secp256k1.Sign(msg, seckey)
    assert err == skycoin.SKY_OK
    err, value = skycoin.secp256k1.VerifyPubkey(pubkey1)
    assert err == skycoin.SKY_OK
    assert value != 0
    err , pubkey2 = skycoin.secp256k1.RecoverPubkey(msg, sig)
    assert pubkey1 == pubkey2


def test_Test_verify_functions():
    pubkey, seckey, hashs, sig = RandX()
    err, value = skycoin.secp256k1.VerifySeckey(seckey)
    assert err == skycoin.SKY_OK
    assert value != 0
    err, value = skycoin.secp256k1.VerifyPubkey(pubkey)
    assert err == skycoin.SKY_OK
    assert value != 0
    err, value = skycoin.secp256k1.VerifySignature(hashs, sig, pubkey)
    assert err == skycoin.SKY_OK
    assert value != 0
    _ = sig
...
def test_Test_Secp256_02s():
    err , pubkey, seckey = skycoin.secp256k1.GenerateKeyPair()
    assert err == skycoin.SKY_OK
    err, msg = skycoin.secp256k1.RandByte(32)
    assert err == skycoin.SKY_OK
    err, sig = skycoin.secp256k1.Sign(msg, seckey)
    assert err == skycoin.SKY_OK
    assert sig != None
...
def test_Test_Secp256_02():
    ...
    err, value = skycoin.secp256k1.VerifySignature(msg, sig, pubkey)
    assert err == skycoin.SKY_OK
    assert value == 1


def test_Test_Secp256_02a():
    err, pubkey1, seckey1 = skycoin.secp256k1.GenerateKeyPair()
    assert err == skycoin.SKY_OK
    err , msg = skycoin.secp256k1.RandByte(32)
    assert err == skycoin.SKY_OK
    err, sig = skycoin.secp256k1.Sign(msg, seckey1)
    assert err == skycoin.SKY_OK
    assert sig != None
    err, value = skycoin.secp256k1.VerifySignature(msg, sig, pubkey1)
    assert err == skycoin.SKY_OK
    assert value == 1
    err, pubkey2 = skycoin.secp256k1.RecoverPubkey(msg, sig)
    assert err == skycoin.SKY_OK
...
def test_Test_Secp256_03():
    err , _, seckey = skycoin.secp256k1.GenerateKeyPair()
    for _ in range(TESTS):
        _, msg = skycoin.secp256k1.RandByte(32)
        _, sig = skycoin.secp256k1.Sign(msg, seckey)
        err , pubkey2 = skycoin.secp256k1.RecoverPubkey(msg, sig)
        assert err == skycoin.SKY_OK
        assert pubkey2 != None
...
# def randSig():
#     err, sig = skycoin.secp256k1.RandByte(65)
#     assert err == skycoin.SKY_OK
#     assert len(sig) == 65
...
#         err, pubkey2 = skycoin.secp256k1.RecoverPubkey(msg, sig)
#         assert pubkey != pubkey2
#         err, value = skycoin.secp256k1.VerifySignature(msg, sig, pubkey2)
#         assert err == skycoin.SKY_OK
...
def test_Test_Deterministic_Keypairs_00():
    for _ in range(64):
        _, seed = skycoin.secp256k1.RandByte(64)
        err, _, pub1, sec1 = skycoin.secp256k1.DeterministicKeyPairIterator(seed)
        assert err == skycoin.SKY_OK
        err, pub2, sec2 = skycoin.secp256k1.GenerateDeterministicKeyPair(seed)
        assert err == skycoin.SKY_OK
        assert pub1 == pub2
        assert sec1 == sec2
...
def test_Test_Deterministic_Keypairs_01():
    for _ in range(64):
        _, seed = skycoin.secp256k1.RandByte(32)
        err, _, pub1, sec1 = skycoin.secp256k1.DeterministicKeyPairIterator(seed)
        assert err == skycoin.SKY_OK
        err, pub2, sec2 = skycoin.secp256k1.GenerateDeterministicKeyPair(seed)
        assert err == skycoin.SKY_OK
...
def Decode(str1):
    err, byt = skycoin.base58.String2Hex(str1)
    assert err == skycoin.SKY_OK
    return byt
...
def test_Test_Deterministic_Keypairs_03():
    ...
    for i in range(int(len(testArray) / 2)):
        ...
        err, _, sec2 = skycoin.secp256k1.GenerateDeterministicKeyPair(seed)
        assert err == skycoin.SKY_OK
        assert sec2 == sec1
...
def test_Test_DeterministicWallets1():
    ...
    for i in range(int(len(testArray) / 2)):
        ...
        err, _, _, seckey2 = skycoin.secp256k1.DeterministicKeyPairIterator(seed)
        assert err == skycoin.SKY_OK
...
def test_Test_Secp256k1_Hash():
    ...
    for i in range(int(len(testArray) / 2)):
        ...
        err, hash3 = skycoin.secp256k1.Secp256k1Hash(hash1)
        assert err == skycoin.SKY_OK
        assert hash2 == hash3
...
def test_Test_Secp256k1_Equal():
    for _ in range(64):
        _, seed = skycoin.secp256k1.RandByte(128)
        err , hash1 = skycoin.secp256k1.Secp256k1Hash(seed)
        assert err == skycoin.SKY_OK
        err, hash2, _, _ = skycoin.secp256k1.DeterministicKeyPairIterator(seed)
        assert hash1 == hash2
...
def test_Test_DeterministicWalletGeneration():
    ...
    for _ in range(1024):
        err, seed, pubkey, seckey = skycoin.secp256k1.DeterministicKeyPairIterator(seed)
        assert err == skycoin.SKY_OK
...
def test_Test_ECDH():
    _, pubkey1, seckey1 = skycoin.secp256k1.GenerateKeyPair()
    _, pubkey2, seckey2 = skycoin.secp256k1.GenerateKeyPair()

    err, puba = skycoin.secp256k1.ECDH(pubkey1, seckey2)
    assert err == skycoin.SKY_OK
    err, pubb = skycoin.secp256k1.ECDH(pubkey2, seckey1)
    assert err == skycoin.SKY_OK
    assert puba != None
    assert pubb != None
    assert puba == pubb
...
def test_Test_Abnormal_Keys2():
    for i in range(len(_testSeckey)):
        err, seckey1 = skycoin.base58.String2Hex(_testSeckey[i])
        assert err == skycoin.SKY_OK
        err, pubkey1 = skycoin.secp256k1.PubkeyFromSeckey(seckey1)
        assert err == skycoin.SKY_OK
        assert pubkey1 != None
        assert seckey1 != None
        assert pubkey1 != None
...
def test_Test_Abnormal_Keys3():
    for i in range(len(_testSeckey)):
        ...
        err, pubkey1 = skycoin.secp256k1.PubkeyFromSeckey(seckey1)
        assert err == skycoin.SKY_OK
        err, seckey2 = skycoin.base58.String2Hex(_testSeckey[random.randint(0, len(_testSeckey) - 1)])
        assert err == skycoin.SKY_OK
        err, pubkey2 = skycoin.secp256k1.PubkeyFromSeckey(seckey2)
        assert err == skycoin.SKY_OK
        assert pubkey1 != None
        assert pubkey2 != None
        err, puba = skycoin.secp256k1.ECDH(pubkey1, seckey2)
