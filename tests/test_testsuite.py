...
def KeysTestDataFromJSON(KeysTestDataJSON):
    ...
    r.Signatures = 0
    if KeysTestDataJSON.get("signatures") is None:
...
def ValidateSeedData(SeedTestData=None, InputTestData=None):
    ...
    for i, s in enumerate(keys):
        ...
        if InputTestData is None and SeedTestData.Keys[i].Signatures != 0:
            return skycoin.SKY_ERROR

        if InputTestData is not None:
            ...
            for j in range(len(InputTestData)):
                ...
                err = skycoin.SKY_cipher_PubKeyFromSig(
                    sig, InputTestData[j], p2)
                if err != skycoin.SKY_OK:
                    return skycoin.SKY_ERROR
                if not(p == p2):
                    return 1
