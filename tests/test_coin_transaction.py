...
def test_TestTransactionVerifyInput():
    ...
    assert skycoin.SKY_coin_Transaction_VerifyInput(
        handle, seckeys) == skycoin.SKY_ERROR
    seckeys = []
    ...
    assert skycoin.SKY_coin_Transaction_PushSignature(
        handle, skycoin.cipher_Sig()) == skycoin.SKY_OK
    seckeys = []
    seckeys.append(ux)
    assert skycoin.SKY_coin_Transaction_VerifyInput(
        handle, seckeys) == skycoin.SKY_ERROR
    ...
    assert skycoin.SKY_coin_Transaction_GetSignatureAt(
        handle, 0, sigs) == skycoin.SKY_OK
    assert skycoin.SKY_coin_Transaction_ResetSignatures(
        handle, 0) == skycoin.SKY_OK
    assert skycoin.SKY_coin_Transaction_PushSignature(
        handle, sigs) == skycoin.SKY_OK
    assert skycoin.SKY_coin_Transaction_PushSignature(
        handle, skycoin.cipher_Sig()) == skycoin.SKY_OK
    seckeys = []
    seckeys.append(ux)
...
def test_TestTransactionSignInputs():
    ...
    assert skycoin.SKY_cipher_ChkSig(a, h, txsig1) == skycoin.SKY_ERROR
    assert skycoin.SKY_cipher_ChkSig(a2, h, txsig0) == skycoin.SKY_ERROR
