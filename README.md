...
To use wrapper for specific node of Skycoin api, use next command:

```bash
$ make update-host NEW_HOST=Specific_Node
```

Example, if you have a Skycoin node running in `http://localhost:6420` just run:

```bash
$ make update-host NEW_HOST=http://localhost:6420
```
